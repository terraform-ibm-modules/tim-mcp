...
CATEGORY_KEYWORDS = {
    "networking": ["vpc", "subnet", "network", "load-balancer", "alb", "nlb", "dns", "transit"],
    "security": ["security", "secrets", "kms", "key-protect", "cbr", "context-based-restrictions", "iam", "access"],
    ...
    "observability": ["observability", "monitoring", "logging", "activity-tracker", "sysdig", "logdna", "metrics"],
...
async def generate_module_index():
    ...
    async with TerraformClient(config, cache) as tf_client:
        ...
        for module in all_modules:
            ...
            try:
                ...
                for submodule in raw_submodules:
                    ...
                    submodule_source_url = f"{source}/tree/main/{submodule_path}" if submodule_path else source
        ...
        print(f"   Categories: {len(set(m['category'] for m in filtered_modules))}")
